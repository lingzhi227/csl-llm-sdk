"""Actual full-rectangle coverage and selected complete-program storage only."""
import hashlib
import json
...
SELECTED = {(v['x'] + 4, v['y'] + 1): v for v in
            json.loads((ROOT / 'selected-programs.json').read_bytes())['programs']}
...
def main():
    covered, selected_covered, rows = set(), set(), []
    ...
    for path in paths:
        ...
        covered |= coordinates
        records = symbols(raw)
        selected = coordinates & set(SELECTED)
        if selected and selected != coordinates:
            raise ValueError('Selected complete program merged with demoted idle code')
        arrays = {}
        alias_views = {}
        if selected:
            for point in sorted(selected):
                expected = expected_arrays(SELECTED[point])
                for name, (size,alignment) in expected.items():
                    arrays[name] = array(records,name,size,alignment)
                if SELECTED[point]['role'] == 4:
                    ...
                    for name,start,end in [('trig',0,512),('rotary_stages',512,2048),
                        ...
                        alias_views[name] = dict(address=base+start,bytes=end-start,
                            evidence='Actual workspace base plus frozen runtime-accessor offset; not a separate ELF symbol')
                if SELECTED[point]['role'] == 8 and SELECTED[point]['head_id'] != 25-point[1]:
                    raise ValueError('Actual selected sink row binds head24-application_y')
            selected_covered |= selected
        else:
            if any('weight_storage' in v['name'] and v['bytes']==24576 for v in records):
                raise ValueError('Unexpected neural weight bank in idle fit role')
            for name,(size,alignment) in expected_arrays(dict(role=0)).items():
                arrays[name] = array(records,name,size,alignment)
        ...
        rows.append(dict(file=path.name, rectangles=sorted(rectangles), selected=sorted(selected),
                         arrays=arrays,source_derived_alias_views=alias_views))
    if covered != ALL or selected_covered != set(SELECTED):
        raise ValueError('All33750 geometry PEs and all89 selected complete programs required')
    atomic_json(ROOT/'placement.json',dict(passed=True,PEs=len(covered),selected_PEs=len(selected_covered),
        full_heads=24,native_sources=16,selected_matrix_PEs=35,application=[750,45],geometry=[762,1172],
        files=rows,runtime_created=False,executable_graph=False,fullgraph_fit_accepted=False,
        passed_scope='placement_and_storage_only',sram_gate_separate=True,
        alias_scope='ELF proves actual workspace/bank extents; accessor offsets require pinned source and separately accepted real alias/archive fixture',
        scope='Selected complete original-role programs only. Demoted PEs prevent execution and fullgraph SRAM acceptance.'))
