"""One selected-program fullfit compile, reusing the accepted owner-correlated job cleanup."""
...
def main():
    ...
    with (ROOT.parent/'hardware.lock').open('a') as lock:
        ...
        try:
            ...
            atomic_json(ROOT/'COMPLETE.json',dict(scope='Selected complete layer3 programs at original coordinates; compile only, never executable',stages=[receipt],all_owned_jobs_released=True,runtime_created=False,full_model=False))
