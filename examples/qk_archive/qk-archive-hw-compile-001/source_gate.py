...
PROFILE=dict(compile_seconds=300,worker_cpu_millicores=1000,worker_memory_bytes=4<<30,
             host_address_space_bytes=4<<30,host_sampled_rss_bytes=1<<30,host_file_bytes=128<<20,
             host_log_bytes=8<<20,candidate_bytes=128<<20,host_disk_reserve_bytes=32<<30,
             application=[4,1],fabric=[762,1172],offset=[4,1],stack_bytes=4096,sram_ceiling_bytes=48128)
def verify():
    ...
    if admission!=dict(candidate=ROOT.name,manifest_sha256=hashlib.sha256(raw).hexdigest(),profile=PROFILE,compile_only=True,automatic_retry=False):
        raise ValueError('Exact four-PE physical fixture compile admission required')
