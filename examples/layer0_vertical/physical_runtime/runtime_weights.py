...
class Session:
    ...
    def __init__(self,io,prepared,root,items,profile):
        ...
        require(len(items)==190 and all(i['symbol']=='weights' for i in items),'Complete weight-only asynchronous upload')
...
def audit_upload(events,items,prepared):
    ...
    require(len(items)==190 and len(events)==4*len(items)+2*len(groups)+1,'Exact finite asynchronous journal size')
