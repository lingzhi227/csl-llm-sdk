...
def audit(evidence,prepared,plan,copies,math,nominal_output):
    ...
    for step in plan['schedule']:
        ...
        for ordinal,stripe in enumerate(plan['stripes']):
            ...
            for xx in range(width):
                chosen=((y+xx)*37+x*17+serial*13)%stripe['valid_rows']
        ...
        def projection(name):return projections['model.language_model.layers.0.'+name+'.weight']
    ...
    return dict(status='all_operator_gates_passed',layer=0,serials=3,positions=2,results=reports,
