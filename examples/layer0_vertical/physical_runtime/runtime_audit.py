"""Separate post-release complete evidence and original Layer0 numeric audit."""
...
def scheduled(copies,observations):
    ...
    yield from transfers('d2h',copies['persistent_copies'],'initialized-zero')
    for serial in (1,2,3):
        if serial==3:
            yield command('reset');yield from transfers('d2h',copies['state_copies'],'s2-reset-state')
            yield from transfers('d2h',copies['persistent_copies'],'reset-zero')
    ...
    with path.open('rb') as f:
        for raw in f:
            ...
            if kind.startswith('weight_') or kind=='all_original_weights_completed':
                ...
                weight_events.append(row);require(len(weight_events)<=1000,'Finite weight event prefix')
...
def control(evidence,plan,transport,copies):
    ...
    def check_states(serial,generation,position,phase):
        for i,item in enumerate(copies['state_copies']):
            state(item,evidence.raw(f's{serial}-{phase}-state-{i:04}.bin'),plan['support']['origin'],serial,generation,position,phase)
    check_states(0,1,0,'initialized');check_states(2,2,0,'reset')
    for label in ('initialized','reset'):
        for i,item in enumerate(copies['persistent_copies']):
            raw=evidence.raw(f'{label}-zero-{i:04}.bin')
            require(not np.any(raw.view('<u2') if item['bits']==16 else raw.view('<u4')),'Complete exact zero persistent state')
    for step in plan['schedule']:
        ...
        for attempt in range(observed['attempts']):
            ...
            require(result['complete']==(attempt==observed['attempts']-1),'Observer loop stops on the first full coherent completion')
        require(result['origin_state']==observed['origin_state'],'Capture observation matches immutable raw')
        for i,item in enumerate(copies['transport_copies']):
            ...
            require(first.tobytes()==second.tobytes(),'Two actual full transport fences identical')
        for i,item in enumerate(copies['diagnostic_copies']):
            ...
            name=item['symbol']
            if serial==3 and not (name.endswith('counters') or name.startswith('linear_') or name=='frame_counters'):
                require(value.tobytes()==evidence.raw(f's1-diagnostic-{i:04}.bin').tobytes(),'Every neural reset replay array exact')
    return dict(all_states=True,all_persistent_zero=True,all_transport=True,all_lifecycle=True,all_reset_neural_replay=True)
...
def main():
    ...
    name='step0-layer0-output.npz';pin={k:plan['reference_inputs'][name][k] for k in ('bytes','sha256')}
    raw=pinned(Path(inputs['reference_root'])/name,pin,65536)
    with np.load(io.BytesIO(raw),allow_pickle=False) as archive:
        require(archive.files==['hidden'] and archive['hidden'].shape==(1,5,5120) and archive['hidden'].dtype==np.dtype('<u2'),
                'Pinned original nominal five-position output boundary')
