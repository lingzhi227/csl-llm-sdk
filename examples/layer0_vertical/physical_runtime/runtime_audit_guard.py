...
def main():
    ...
    os.sched_setaffinity(0,{0});resource.setrlimit(resource.RLIMIT_AS,(512<<20,profile['address_space']))
    ...
    with (ROOT.parent/'hardware.lock').open('a') as lock:
        ...
        proc=None;error=None;started=time.monotonic();peak=0
        try:
            ...
            atomic_json(ROOT/'audit-supervisor.json',dict(error=error,cleanup_error=cleanup_error,
                ...
                wall_seconds=time.monotonic()-started,maximum_sampled_RSS=peak,profile=profile,SDK=False))
