"""Complete finite original Layer0 host sequence for positions0,1,reset0.
...
def capture(io,prepared,root,plan,transport,copies,profile,timing):
    ...
    reset_signatures={};replay_count=0;zero_checks=0
    ...
    def transport_fence(serial,resets):
        ...
        for snapshot in range(2):
            ...
            require(points==34800,'All application coordinates in each transport fence')
    ...
    try:
        ...
        parameters('d2h','initial-retention');states(0,1,0,'initialized');persistent_zero('initialized')
        for step in SCHEDULE:
            ...
            if serial==3:
                ...
                io.launch('reset');states(2,2,0,'reset');persistent_zero('reset')
            ...
            for i,item in enumerate(copies['diagnostic_copies']):
                ...
                # Reset must reproduce all neural values, not cumulative
                # lifecycle counters or scheduling-dependent queue peaks.
                name=item['symbol']
                arithmetic=not (name.endswith('counters') or name.startswith('linear_') or name=='frame_counters')
                if arithmetic and serial==1:reset_signatures[i]=receipt['sha256']
                elif arithmetic and serial==3:
                    require(reset_signatures[i]==receipt['sha256'],'Reset replay exact retained arithmetic: '+name)
                    replay_count+=1
            require(hidden_output is not None,'Actual Layer0 hidden output is retained')
        ...
        result=dict(status='captured_pending_independent_numeric_audit',completed=completed,observations=observations,
            ...
            weight_upload=io.weight_result)
