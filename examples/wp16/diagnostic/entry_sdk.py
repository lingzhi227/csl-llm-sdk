...
from qwen38.wp16_sdk_admission import (
    IMAGE, PROFILE, THREAD_ENV, WORK, image_stamp, require, verify, verify_compiled)


def main():
    require(len(sys.argv) == 2 and sys.argv[1] in ('compile', 'simulate'), 'Exact SDK stage')
    stage = sys.argv[1]
    ...
    require(actual == {'memory.max': '1073741824', 'memory.swap.max': '0', 'pids.max': '128'}, 'Actual SDK hard limits')
    ...
    command += [str(IMAGE), '/python/python-x86_64/bin/python', 'container_entry.py', stage]
