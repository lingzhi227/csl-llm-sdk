"""One scoped CPU0 compiler/simulator pair; retain every failed attempt, no retry."""
import hashlib
import json
import os
...
from qwen38.wp16_sdk_admission import (
    PROFILE, STEPS, THREAD_ENV, WORK, image_stamp, require, verify, verify_compiled)
...
def main():
    ...
    require(not any((WORK / name).exists() for name in
                    ('supervisor.json', 'out', 'device-evidence', 'compile.log', 'simulate.log',
    ...
    budget = Budget(job_ram=PROFILE['memory_bytes'])
        ...
        for step in STEPS['steps']:
            try:
                ...
                if step['name'] == 'simulate':
                    # The compiler unit has been stopped and its cgroup emptied.
                    require(report['steps'][-1]['status'] == 'passed' and
                            report['steps'][-1]['cleanup'] == 'owned_unit_quiescent', 'Serial compiler cleanup gate')
            ...
            fields = dict(RemainAfterExit='yes', MemoryMax=str(PROFILE['memory_bytes']), MemorySwapMax='0',
            ...
            try:
                ...
                while True:
                    ...
                    if group_name:
                        ...
                        try:
                            ...
                            require(actual == {'memory.max': '1073741824', 'memory.swap.max': '0', 'pids.max': '128'}, 'Runtime actual SDK limits changed')
                ...
                require(item['affinity_live_sample_count'] > 0 and item.get('last_actual_limits') ==
                        {'memory.max':'1073741824','memory.swap.max':'0','pids.max':'128'}, 'Positive native mask and actual cgroup observations')
                if step['name'] == 'compile':
                    verify_compiled(WORK)
                else:
                    outcome = json.loads((WORK / 'device-evidence/result.json').read_bytes())
                    require(outcome['status'] == 'diagnostic_passed' and outcome['normal_stop'] is True, 'All SDK gates and normal stop required')
