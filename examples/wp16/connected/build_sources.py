...
from qwen38.wp16_reuse_layout import ARRAYS, EDGES, FLOAT32, FIELD_INDICES, budget, sequence
...
def main():
    ...
    if body.count('// @connected_lifecycle@') != 1:
        raise ValueError('Exactly one lifecycle source splice required')
    body = body.replace('// @connected_lifecycle@', (HERE / 'lifecycle.csl.inc').read_text().rstrip())
    ...
    interfaces = dict(arrays={name:dict(bits=bits,counts=list(counts)) for name,(bits,counts) in ARRAYS.items()}, edges=EDGES, field_indices=FIELD_INDICES, wire_version=2)
