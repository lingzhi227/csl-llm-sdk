...
def valid():
    ...
    origin=[]
    for owner in range(136):origin.append(rec(len(origin)+1,4,c.header(10,3,owner),network=0x400128,subject=owner))
    peer=[]
    for group in range(8):
        peer.append(rec(len(peer)+1,4,c.header(11,2,group,128*group),network=0x600128,subject=group))
        for n,offset in [(31,0),(31,23),(26,46)]:
            h=c.header(9,2,group,offset);origin.append(rec(len(origin)+1,4,h,length=n,network=0x400120+n,subject=group))
...
def main():
    ...
    expected=expected.replace('var pending_source:[*]u32;','var pending_source:[*]u32;\n// Candidate B owns the entire injected frame until the existing sent callback.\nvar transmit_frame=@zeros([32]u32);')
    delta=json.loads((Path(__file__).parent/'TX-DELTA.json').read_text())
    expected=expected.replace(delta['old_call'],delta['new_call'])
    for change in json.loads((Path(__file__).parent/'RX-DELTA.json').read_text())['changes']:
        assert expected.count(change['old'])==1
        expected=expected.replace(change['old'],change['new'])
    for change in json.loads((Path(__file__).parent/'CONTROL-DELTA.json').read_text())['changes']:
        assert expected.count(change['old'])==1
        expected=expected.replace(change['old'],change['new'])
    promotion=json.loads((Path(__file__).parent/'NATIVE-PROMOTION-DELTA.json').read_text())
    sha=lambda raw:hashlib.sha256(raw).hexdigest()
    assert sha(expected.encode())==promotion['parent_packet_sha256']
    assert sha(current.encode())==promotion['candidate_packet_sha256']=='06083863a5c55d92bc79c4ba3aad07400617c6faa6e4bd9246fe8abe35c7fd85'
    qualified_raw=(Path(__file__).parent/'QUALIFIED-PACKET-MANIFEST.json').read_bytes()
    assert sha(qualified_raw)==promotion['qualified_compile_manifest_sha256']
    assert json.loads(qualified_raw)['files']['packets.csl']==dict(bytes=len(current.encode()),sha256=sha(current.encode()))
    for name,row in promotion['unchanged'].items():
        raw=(Path(__file__).parent/name).read_bytes();assert len(raw)==row['bytes'] and sha(raw)==row['sha256'],name
    assert current.count('transmit_frame[0]=')==1 and current.count('copy_words(pending_source,@ptrcast([*]u32,&transmit_frame[1]),pending_length);')==1
    assert current.count('@mov32(frame_out,frame_src,.{.async=true,.activate=sent_task_id});')==1
    assert 'mp.send_message(' not in current
    print(json.dumps(dict(passed=True,positive=2,negative=len(cases),partial_first_error=1,transport_delta_exact=True,qualified_native_packet_exact=True,complete_fixture_unqualified=True,internal_frame_words=32,declared_frame_bytes=128,device_buffer_lifetime_proven=False,SDK_invocations=0,hardware_invocations=0)))
