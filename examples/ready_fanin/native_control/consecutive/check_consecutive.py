...
def check(state,events,observed,received_buffer,tx_evidence,rx_evidence,transmitted_frame,source_payload,tail_normal_events,consumption_events):
    ...
    rx,tx=state
    require(all(s[0]==2 and s[14]==s[15]==0 for s in state),'Initialized static route and no harness errors')
    ...
    return dict(passed=True,lengths=LENGTHS,deliveries=2,source_completions=2,callback_events=events,native_consumption=consumption_events,full_u32_preserved=True,hardware=False,message_passing_fabric_test=False)
