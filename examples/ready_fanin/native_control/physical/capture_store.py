...
import numpy as np
SPECS={'initial':(356,4),'origin':(1,3072),'peer':(1,1536),'producers':(136,48),'final-status':(178,8),**{f'progress-{i:02d}':(178,8) for i in range(8)}}
...
def save(root,label,value):
    ...
    if len(paths)>=13 or len(raw)>131072 or sum(p.stat().st_size for p in paths)+len(raw)>1048576:raise ValueError('Finite raw budget')
