"""Durable immutable bit-preserving NPZ receipts for the finite lifecycle capture."""
from pathlib import Path
import hashlib,io,json,os
import numpy as np
SPECS={'initial-state':(3,64),'routing':(3,8),'observed':(3,256),'tx-proof':(3,384),'rx-banks':(3,31),'live-frames':(3,32),'live-sources':(3,31),'row-archive':(3,128),'final-state':(3,64),**{'state-'+str(i):(3,64) for i in range(8)}}
...
def save(root,label,value):
    ...
    if len(paths)>=17 or len(raw)>8192 or sum(p.stat().st_size for p in paths)+len(raw)>1048576:raise ValueError('Finite raw budget')
