"""Capture the unchanged finite fixture, reading only independent row1 sinks."""
import json,os,time
...
def capture(root,runner,DataType,Order):
    ...
    stopped=False;error=None;summary=None;partial=None
    ...
    def copy(label,name,x,y,w,h,count):
        ...
        if copies>=13 or count>3072 or w*h*count>136*48:raise ValueError('Finite rectangle')
        ...
        if host_bytes>101504:raise ValueError('Aggregate raw host bound')
    ...
    try:
        event('capture_start');symbols={n:runner.get_id(n) for n in ['state','diagnostic_status','diagnostic_records']}
        ...
        result=dict(status='passed' if error is None and stopped else 'failed',normal_stop=stopped,copies=copies,launches=launches,host_slot_bytes=host_bytes,journal_events=sequence,captures=receipts,summary=summary,partial_summary=partial,hardware=True,neural_math=False,rounds=1,reset=False,seconds=time.monotonic()-started,message=None if error is None else str(error)[:512])
