"""Bounded534PE physical capture. All packet progress is on device."""
from pathlib import Path
import hashlib,json,os,time
from schema import SCHEMA,validate

# label, exported symbol, x, y, width, height, u32 words per PE.
FULL=[('state','state',0,0,178,3,32),
      ('input_a_status','input_a_status',0,0,178,3,8),
      ('input_b_status','input_b_status',0,0,178,3,8),
      ('output_status','output_status',0,0,178,3,8),
      ('diagnostic_status','diagnostic_status',0,0,178,2,8),
      ('origin_records','diagnostic_records',0,0,1,2,3072),
      ('peer_records','diagnostic_records',2,0,1,2,1536),
      ('producer_records','diagnostic_records',40,0,136,2,48),
      ('idle1_records','diagnostic_records',1,0,1,2,16),
      ('idle3_records','diagnostic_records',3,0,37,2,16),
      ('idle176_records','diagnostic_records',176,0,2,2,16),
      ('relay_records','relay_records',0,2,178,1,2176),
      ('row_packets','row_packets',0,0,1,1,768)]
SPECS={'initial-state':('state',0,0,178,3,32)}
for i in range(8):
    SPECS[f'poll-{i}-state']=('state',0,0,178,3,32)
    SPECS[f'poll-{i}-diag']=('diagnostic_status',0,0,178,2,8)
for cycle in (0,1):
    for name,symbol,x,y,w,h,n in FULL:SPECS[f'final-{cycle}-{name}']=(symbol,x,y,w,h,n)
assert len(SPECS)==43 and sum(w*h*n*4 for symbol,x,y,w,h,n in SPECS.values())==4261504
assert sum(w*h*n*4 for name,symbol,x,y,w,h,n in FULL)==1777600
...
def assemble(records,cycle):
    tiles={f'{x},{y}':{} for y in range(3) for x in range(178)}
    ...
    return dict(schema=SCHEMA,first=40,count=136,qualification_gate=False,tiles=tiles)


def initialized(words):
    if len(words)!=17088:raise ValueError('Initial state extent')
    for i in range(534):
        x,y=i%178,i//178;role=5 if y==2 else 4 if y==1 else 1 if x==0 else 2 if x==2 else 3 if 40<=x<=175 else 0
...
def settled(state,diag):
    if any(state[i*32+4] for i in range(534)):return False
    if not all(state[i*32]==2 and state[i*32+2]==1 for i in range(534)):return False
    for y in range(2):
        for x in range(178):
            s=diag[(y*178+x)*8:(y*178+x+1)*8]
            n=185 if x==0 else 81 if x==2 else 3 if 40<=x<=175 else 0
            cap=192 if x==0 else 96 if x==2 else 3 if 40<=x<=175 else 1
...
def capture(root,runner,MemcpyDataType,MemcpyOrder):
    ...
    events=0;copies=0;host_bytes=0;launches=0;raw_bytes=0
    receipts=[];records={};error=None;normal_stop=False;checks={};started=time.monotonic()
    atomic(root/'monitor-deadline.json',dict(active=True,deadline_monotonic=started+90,scope='capture and normal context exit after context entry'))
    ...
    def read(label):
        ...
        if label not in SPECS or label in records or copies>=43:raise ValueError('Capture count/label')
        symbol,x,y,w,h,n=SPECS[label];size=w*h*n*4
        if host_bytes+size>4261504:raise ValueError('Capture host-byte bound')
        ...
        if len(raw)>2097152 or raw_bytes>6291456:raise ValueError('Raw evidence bound')
    ...
    try:
        ...
        for i in range(8):
            ...
            if any(state[j*32+4] for j in range(534)) or settled(state,diag):break
        ...
        if runner is not None:
            ...
            try:
                ...
                if not normal_stop:raise ValueError('Actual context exit not returned')
                event('stop_exit');atomic(root/'monitor-deadline.json',dict(active=False,reason='normal_exit_returned'))
            except BaseException as exc:
    ...
    result=dict(status='passed' if error is None and normal_stop else 'failed',normal_stop=normal_stop,
                ...
                hardware=True,PEs=534,producers=136,requests=8,fragments=24,halfwords=1024,original_neural_epochs=0)
...
def validate_saved(root):
    ...
    for row in result['captures']:
        ...
        path=root/name
        if path.is_symlink() or path.stat().st_size>2097152:raise ValueError('Capture regular extent')
    ...
    if len(records)!=result['copies'] or result['copies']>43 or host!=result['host_bytes'] or host>4261504 or total!=result['raw_bytes'] or total>6291456:raise ValueError('Capture accounting')
    if result['launches']!=2 or result['H2D']!=0 or result['hardware'] is not True or result['original_neural_epochs']!=0:raise ValueError('Execution scope')
