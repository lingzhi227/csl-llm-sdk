"""Read only actual physical ELF placement, SRAM and disjoint diagnostic banks."""
...
def main():
    ...
    for p in sorted((ROOT/'compiled/out').rglob('*')):
        if p.is_file():
    ...
    for p in sorted((ROOT/'compiled/out/bin').glob('*.elf')):
        raw=p.read_bytes();records=symbols(raw);reader=ELFMemory(p);assert tuple(reader.get_fabric_dimensions())==(762,1172)
    ...
    result=dict(passed=all(v['sram']['passed'] for v in programs),files=files,application=programs,geometry=[762,1172],application_shape=[178,2],PEs=356,runtime_created=False,DSR_allocation_proven=False)
    with (ROOT/'placement.json').open('x') as f:json.dump(result,f,indent=2);f.write('\n')
