"""Compile the unchanged 356-PE synthetic concurrent READY fixture; no CS3 runtime."""
...
def main():
    ...
    with tarfile.open(path) as archive:
        ...
        if len(members)>2048 or sum(m.size for m in members)>64<<20:raise ValueError('Compile inventory budget')
        app=[m for m in members if m.isfile() and '/out/bin/' in m.name and m.name.endswith('.elf')]
        if not 1<=len(app)<=356:raise ValueError('Bounded finite application programs')
        for member in app:
    ...
    if not passed:raise ValueError('Finite fan-in fixture SRAM gate')
    verify()
