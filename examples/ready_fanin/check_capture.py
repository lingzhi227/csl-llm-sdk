...
def check(status,origin,peer,producers,*,require_complete=True):
    ...
    for r in decoded[0][:-1]:
        require(r[1]==4 and r[4]==0x00400100+r[3],'origin exact network header')
        h=r[5:13]
        if h[0]==10:
    ...
    for at,r in enumerate(p[:-1]):
        h=r[5:13];group=h[6]
        if r[1]==4:
            require(r[3]==8 and r[4]==0x00600108 and 0<=group<8 and h==header(11,2,group,group*128),'peer request identity')
