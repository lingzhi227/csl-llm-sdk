"""One complete native graph compile, reusing the accepted owner-correlated job cleanup."""
...
def main():
    ...
    with (ROOT.parent/'hardware.lock').open('a') as lock:
        ...
        try:
            ...
            atomic_json(ROOT/'COMPLETE.json',dict(scope='Complete native layer3 graph compile only; physical runtime requires separate admission',stages=[receipt],all_owned_jobs_released=True,runtime_created=False,full_model=False))
