"""WP14 full5120 Q/K projection, raw32 device handoff and trained preprocessing."""
import hashlib
import json
import os
...
from qwen38.wp14_layout import ARRAYS, sequence, budget
from qwen38.projection_handoff import SLABS
...
assert profile['scope']=='wp14-original-hidden-qk-handoff-rms-rope'
assert profile['pes']==5 and profile['producer_slabs']==[0,1,4,5]
assert profile['case_indices']==[0] and profile['position']==1
...
try:
    ...
    for call, case_index in enumerate(cases,1):
        ...
        consumer_checks=consumer.handoff_and_compute()
        retention_checks=[]
        for pe in range(pes):
            ...
            retention_checks.append(dict(pe=pe,global_slab=SLABS[pe],after_consumer=True,
        ...
        np.savez(root/'device-observations.npz',**observed)
        print('PASS',case,'QK projections/handoff/trained RMS/RoPE',list(SLABS),flush=True)
...
(root/'result.json').write_text(json.dumps(dict(passed=True,cases=passed_calls,
    scope=profile['scope'],producer_slabs=list(SLABS),position=1,
    full_head_qualified=False,full_model_qualified=False,
    selected_qk_composition_qualified=True,normal_stop=True,
