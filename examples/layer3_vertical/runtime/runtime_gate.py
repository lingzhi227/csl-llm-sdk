...
def verify(kind):
    ...
    if bindings['application']!=[29,1160] or bindings['inputs']['compiled']!=inputs['compiled_pin']:
        raise ValueError('Actual revised vertical compile and bound application identity')
    if Path(inputs['compiled_path']).parent.name!='layer3-vertical-artifact-review001':
        raise ValueError('New corrected same-artifact Layer3 application inspection is required')
    pinned(inputs['compiled_path'],inputs['compiled_pin'],8<<20)
    if Path(inputs['prepared_root']).name!='layer3-vertical-prepared-002':
        raise ValueError('Original Layer3 vertical preparation; unused bottom candidate cannot substitute')
...
def artifact(inputs):
    ...
    if Path(inputs['artifact_path']).parent.name!='layer3-vertical-hw-compile-001':
