...
PARAMETERS={'weights','norm_gains','qk_gains','frequencies'}
CONTROL={'transport_config','route','descriptor','state','transport_status'}
...
def copy_spec(symbol,dtype,box,**extra):
    ...
    assert 0<=x and 0<=y and w>0 and h>0 and x+w<=29 and y+h<=1160
...
def build(plan,transport,host,typed,observer_limit=128):
    ...
    for category in ('config_uploads','matrix_uploads','parameter_uploads'):
        for original in host[category]:
            ...
            upload.append(item)
    states=[copy_spec('state','u32',b) for b in rectangles({xy:12 for xy,r in declared.items() if r['role']!=9})]
    transport_copies=[]
    for router,count in ((False,8),(True,32)):
        transport_copies.extend(copy_spec('transport_status','u32',b,router=router)
            for b in rectangles({xy:count for xy,r in declared.items() if (r['role']==9)==router}))
    diagnostic=[]
    for e in typed['exports']:
        if e['name'] in PARAMETERS|CONTROL:continue
        points={}
    ...
    x,y=plan['observer'];observer=copy_spec('observer','u32',[x,y,1,1,32])
    ...
    return dict(scope='Original complete Layer3 host source for0,1,reset0; no runtime admission',uploads=upload,
