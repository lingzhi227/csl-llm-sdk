"""WP15 original5120 projection -> trained QK -> persistent attention, two tokens."""
import hashlib
import json
import os
...
from qwen38.wp15_layout import ARRAYS, sequence, budget
SLABS=tuple(range(8))
from consumer_checks import ConsumerChecks
import numpy as np
...
assert profile['scope']=='wp15-original-hidden-persistent-attention'
assert profile['pes']==10 and profile['producer_slabs']==list(range(8))
assert profile['case_indices']==[0,1] and profile['positions']==[0,1] and profile['request_generations']==[1,1]
...
assert json.loads((root/'physical-sequence.json').read_text())==physical_sequence
assert profile['traffic']==budget()
operation_index=0
journal = []
...
def transfer(direction,name,pe,count,bits=32,values=None,dtype=np.uint32):
    global copies,host_bytes,payload_bytes,operation_index
    expected=physical_sequence[operation_index]
    assert expected['kind']=='copy'
    assert (direction,name,pe,count,bits)==tuple(expected[k] for k in ('direction','name','pe','count','bits'))
    if count*4>65536:
    ...
    copies+=1;operation_index+=1;host_bytes+=count*4;payload_bytes+=count*bits//8
...
def launch(name):
    global launches,operation_index
    expected=physical_sequence[operation_index]
    assert expected==dict(kind='launch',name=name,ordinal=launches+1)
    event('launch_enter',name=name,ordinal=launches+1)
    runner.launch(name,nonblock=False)
    launches+=1;operation_index+=1
    event('launch_exit',name=name,ordinal=launches)
...
try:
    ...
    norm_weights=np.concatenate([decode(weights_dir/mapping[n]['file'],(256,),mapping[n]['file_offset']) for n in ('q_norm','k_norm')]).tolist()
    consumer=ConsumerChecks(root,transfer,launch,observed,norm_weights)
    consumer.initialize()
    for pe in range(pes):
    ...
    for call, case_index in enumerate(cases,1):
        ...
        for tile in range(46):
            ...
            if tile in (0,44):
                for pe in range(pes):
                    ...
                    assert state.tolist()==exact_state(call,tile+1,offset+valid,1,SLABS[pe],call-1)
                    check_fp32(partial[1:-1],matrix[SLABS[pe]*128:(SLABS[pe]+1)*128],x,offset+valid)
        launch('finalize')
        for pe in range(pes):
            ...
            np.savez(root/'device-observations.npz',**observed)
            start=SLABS[pe]*128;slab=matrix[start:start+128]
            assert out[0]==1234.5 and out[-1]==-4321.25 and cast[0]==0xa55a and cast[-1]==0x5aa5
            assert state.tolist()==exact_state(call,46,5120,2,SLABS[pe],call-1)
            ...
            summaries.append(dict(call=call,pe=pe,global_slab=SLABS[pe],
        ...
        consumer_checks=consumer.handoff_and_compute(case)
        for pe in range(pes):
            retained_fp32=transfer('d2h','output_storage',pe,130,dtype=np.float32)
            retained_bf16=transfer('d2h','bf16_storage',pe,130,16)
            observed[f'{call}_{pe}_retained_fp32']=retained_fp32
            observed[f'{call}_{pe}_retained_bf16']=retained_bf16
            np.savez(root/'device-observations.npz',**observed)
            assert np.array_equal(retained_fp32.view(np.uint32),observed[f'{call}_{pe}_fp32'].view(np.uint32))
            assert np.array_equal(retained_bf16,observed[f'{call}_{pe}_bf16'])
            slab=matrix[SLABS[pe]*128:(SLABS[pe]+1)*128]
            ...
            retention_checks.append(dict(call=call,pe=pe,global_slab=SLABS[pe],after_consumer=True,
                fp32_words_exact=130,bf16_words_exact=130,resident_weight_words_exact=14338))
            (root/'producer-retention-checks.json').write_text(json.dumps(retention_checks,indent=2)+'\n')
        launch('release')
        for pe in range(pes):
            state=transfer('d2h','state',pe,13)
            observed[f'{call}_{pe}_released_state']=state
            assert state.tolist()==exact_state(call,46,5120,4,SLABS[pe],call)
        consumer.verify_release()
        passed_calls.append(case)
        np.savez(root/'device-observations.npz',**observed)
        print('PASS',case,'original projections/trained QK/persistent attention',list(SLABS),flush=True)
...
assert operation_index==len(physical_sequence)
(root/'result.json').write_text(json.dumps(dict(passed=True,cases=passed_calls,
    scope=profile['scope'],producer_slabs=list(SLABS),positions=[0,1],request_generations=[1,1],contraction_ids=[1,2],
    full_head_qualified=False,full_model_qualified=False,
    selected_two_token_attention_chain_qualified=True,normal_stop=True,
    input_sha256=profile['input_sha256'],copies=copies,host_bytes=host_bytes,
    payload_bytes=payload_bytes,launches=launches,source_and_conditional_gates_separate=True,
    producer_retention_checked_after_consumer=True),indent=2)+'\n')
