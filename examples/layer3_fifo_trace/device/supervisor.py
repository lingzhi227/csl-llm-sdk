...
def main():
    ...
    with (ROOT.parent/'hardware.lock').open('a') as lock:
        ...
        try:
            ...
            atomic_json(ROOT/'COMPLETE.json',dict(scope='Complete original layer3 source graph compile only',stages=[receipt],all_owned_jobs_released=True,runtime_created=False,full_model=False))
