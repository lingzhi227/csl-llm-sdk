...
def stage(root, name, script, timeout):
    ...
    error = None
    code = None
    journal_size = None; journal_advanced = None
    maximum_journal_idle_seconds = 0.0
    ...
    try:
        with (root / (name + ".log")).open("x") as log:
            ...
            while proc.poll() is None:
                ...
                if len(sizes)>10000 or sum(sizes)>PROFILE["candidate_bytes"]:
                    raise RuntimeError("Full compile candidate storage budget exceeded")
                if time.monotonic() - started > timeout:
                    raise TimeoutError(name + " host deadline")
                # A blocked observer read cannot extend the absolute compute-monitor budget.
                monitor = root / "monitor-deadline.json"
                if name == "run" and monitor.exists():
                    if monitor.stat().st_size > 4096:raise RuntimeError("Monitor control record bound")
                    window=json.loads(monitor.read_bytes())
                    if window['active'] and time.monotonic() > window['deadline_monotonic']:
                        raise TimeoutError("Absolute global monitor deadline")
                # Once capture starts, even one blocked copy/launch has a finite
                # progress budget. Setup before the first journal keeps its full deadline.
                journal = root / "evidence/journal.jsonl"
                if name == "run" and journal.exists():
                    size = journal.stat().st_size; now = time.monotonic()
                    if size > 8 << 20:
                        raise RuntimeError("Capture journal byte bound")
                    if journal_size != size:
                        journal_size = size; journal_advanced = now
                    else:
                        idle = now - journal_advanced
                        maximum_journal_idle_seconds = max(maximum_journal_idle_seconds, idle)
                        if idle > PROFILE["capture_progress_seconds"]:
                            raise TimeoutError("Capture journal exceeded progress budget: "+str(PROFILE["capture_progress_seconds"])+"seconds")
        ...
        receipt = dict(stage=name, exit_code=code, error=error, jobs=records,
                       ...
                       wall_seconds=time.monotonic()-started, maximum_journal_idle_seconds=maximum_journal_idle_seconds)
