...
import math
from qualified_linear_math.rms_numerics import f32,bits,from_bits,bf16_rne,bf16_of_real,gamma,U,TINY,EPSILON,close,PROBE_BITS
from qualified_linear_math.gated_numerics import expanded,silu
CHANNELS=384
POLICY={'name':'chain-preprocess384-domain-v2','channels':384,'history_width':4,'epsilon':EPSILON,
 'scope':'One actual captured128Q/128K/128V head; same perstage quantitative gates, explicit input-domain version2',
 ...
 'domain':'Finite original BF16 operands and each actual exponential argument in[-48,5]; justified by EXP-DOMAIN-V2.json; no unrestricted full-model domain claim',
 'exponential':'custom domain[-48,5] range reduction by splitln2, degree8 FP32 Horner polynomial, exact normal power-of-two scale; gate unchanged',
...
def validate(token,weights,history,observed):
    ...
    conversions['actual_exp_arguments_in_domain']=all(-48<=x<=5 for x in exp_arguments)
