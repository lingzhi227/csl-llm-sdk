...
from attention_numeric_views import Views
from qualified_attention_math.projection_checks import decode,encode,equal,normal,norm_gate,nonlinear_gate
from qualified_attention_math.local_bounds import local_bounds
from qualified_attention_math.exact_arithmetic import exact_dot
from qualified_attention_math.qk_native_conditional import validate_qk
from qualified_attention_math.attention_conditional import validate_attention
...
def audit(evidence,prepared,physical,copies,math,nominal_output):
    ...
    plan=json.loads((Path(__file__).resolve().parent/'attention-logical-plan.json').read_bytes())
    views=Views(evidence,prepared,physical,copies,plan)
    require(nominal_output.shape==(2,5120) and nominal_output.dtype==np.dtype('<u2'),'Pinned original nominal output positions0/1')
    original_input = evidence.input_rows()
    ...
    for epoch,reset,position,input_row in SCHEDULE:
        ...
        equal(saved[0], evidence.input(epoch), 'Exact actual preceding-layer device hidden boundary')
        equal(evidence.input(epoch), original_input[input_row], 'Exact causal input or reset replay')
