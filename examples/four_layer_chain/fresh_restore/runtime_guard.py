...
from runtime_liveness import Parent as LivenessParent
...
def main():
    ...
    os.sched_setaffinity(0,{0});resource.setrlimit(resource.RLIMIT_AS,(8<<30,8<<30))
    ...
    with (ROOT.parent/'hardware.lock').open('a') as lock:
        ...
        child_memory_peaks={key:0 for key in ('VmSize','VmPeak','VmRSS','VmHWM','Threads')};last_child_memory=None
        liveness=LivenessParent();diagnostics=None
        def diagnostic_report():
            # Diagnostics must never bypass the existing owned-job cleanup.
            try:return liveness.report()
            except BaseException as exc:return dict(diagnostic_error=repr(exc)[:1024],success_or_durability_proof=False)
        def limits():
        ...
        try:
            ...
            env.update(liveness.environment())
            with (ROOT/'run.log').open('x') as log:
                ...
                while proc.poll() is None:
                    ...
                    if int(memory['MemAvailable'].split()[0])*1024<profile['host_available_memory_reserve_bytes']:
                        raise RuntimeError('Host available RAM reserve')
                    if shutil.disk_usage(ROOT).free<profile['host_disk_reserve_bytes']:raise RuntimeError('Host disk reserve')
                    metadata_bytes=profile['nested_source_bytes']
                    ...
                    try:
                        ...
                        last_child_memory={key:int(fields.get(key,'0').split()[0])*(1 if key=='Threads' else 1024)
                            for key in ('VmSize','VmPeak','VmRSS','VmHWM','VmSwap','Threads')}
                        for key in child_memory_peaks:child_memory_peaks[key]=max(child_memory_peaks[key],last_child_memory[key])
            ...
            # Capture before cancellation, then drain after reap. Neither path
            # reads the possibly blocked output filesystem for diagnostics.
            diagnostics=diagnostic_report()
            released=cleanup(ROOT,'run',baseline,proc,terminate_fn=terminate_owned)
            diagnostics=diagnostic_report();liveness.close()
            atomic_json(ROOT/'run-audit.json',dict(profile=profile,child_pid=None if proc is None else proc.pid,
                ...
                sampled_child_memory_peaks=child_memory_peaks,last_sampled_child_memory=last_child_memory,
                maximum_monitor_poll_gap_seconds=monitor.max_poll_gap,independent_liveness=diagnostics,**released))
        ...
        if result['completed']!=([1,2,3] if result['mode']=='baseline' else [1]) or not result['normal_stop'] or not life['normal_stop'] or life['primary_error'] or life['stop_error']:
            raise RuntimeError('Complete causal capture and normal stop required')
        durable=result.get('journal_durability',{})
        if (durable.get('final') is not True or durable.get('events')!=result['journal_events'] or
                durable.get('bytes')!=result['journal_bytes'] or
                json.loads((ROOT/'journal-durability.json').read_bytes())!=durable):
            raise RuntimeError('Complete final journal sync is required before physical completion')
        atomic_json(ROOT/'COMPLETE.json',dict(scope='Complete original four-layer raw capture, pending independent numerical/restore audit',
