...
N=128
POLICY={'name':'layer0-gated-rms128-expanded-domain-candidate-v1','epsilon':EPSILON,'size':N,
 'domain':'finite BF16 x/gain, abs(x)<=2, abs(gain)<=2, BF16 gate z in[-48,48]; no overflow/subnormal correctness claim',
...
def reference(x,w,z):
    ...
    if any(not math.isfinite(v) or abs(v)>limit or bits(v)&65535 for a,limit in ((x,2),(w,2),(z,48)) for v in a):raise ValueError('Outside BF16 domain')
