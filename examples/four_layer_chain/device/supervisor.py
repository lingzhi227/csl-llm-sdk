"""One complete original0-3 compile, reusing the accepted owner-correlated job cleanup."""
...
def main():
    ...
    with (ROOT.parent/'hardware.lock').open('a') as lock:
        ...
        try:
            ...
            verify()
            if not json.loads((ROOT/'sram.json').read_bytes())['passed'] or not json.loads((ROOT/'compiled.json').read_bytes())['measurement_complete'] or not json.loads((ROOT/'compiled.json').read_bytes())['all_measured_sram_passed']:
                raise RuntimeError('Static chain artifact gate failed')
            atomic_json(ROOT/'COMPLETE.json',dict(scope='Complete original original0-3 layer chain compile only; separate runtime admission required',stages=[receipt],all_owned_jobs_released=True,runtime_created=False,full_model=False))
