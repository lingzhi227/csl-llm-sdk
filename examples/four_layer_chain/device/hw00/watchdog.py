...
def group_members(pgid):
    ...
    if len(p.stdout)>1<<20:raise RuntimeError('Process inventory bound')
    return [int(pid) for line in p.stdout.splitlines() for pid,gid in [line.split()] if int(gid)==pgid]
...
def group_rss(pgid):
    ...
    for pid in group_members(pgid):
        ...
        total+=sum(int(line.split()[1])*1024 for line in lines if line.startswith('VmRSS:'))
    return total
...
def terminate(proc):
    ...
    # Inspect descendants even when the direct parent already exited.
    if group_members(proc.pid):
        os.killpg(proc.pid,signal.SIGTERM);time.sleep(.5)
        if group_members(proc.pid):os.killpg(proc.pid,signal.SIGKILL);time.sleep(.5)
    if group_members(proc.pid):raise RuntimeError('Owned child process group remains')
    ...
    try:
        with (root / (name + ".log")).open("x") as log:
            ...
            while proc.poll() is None:
                ...
                # Include the streaming inspection subprocess in the same
                # sampled1GiB host envelope; AS4GiB remains inherited and hard.
                if group_rss(proc.pid)>PROFILE['host_sampled_rss_bytes']:
                    raise RuntimeError('Owned host process group exceeds sampled1GiB RSS')
                time.sleep(1)
