...
def expected_arrays(params, exports):
    role=params['role'];root=role==1 and params['ordinal']==0
    linear_root=root and params['matrix_kind']<=4
    endpoint=root or role in (2,3,4,5,6,8,9)
    if role==10:
    ...
    for export in exports:
        selectors=export['roles']
        active=role in selectors or ('matrix_root' in selectors and root) or ('linear_matrix_root' in selectors and linear_root)
        if not active:continue
        count=export['count'];count=count[str(role)] if isinstance(count,dict) else count
        size=2 if export['dtype']=='u16' else 4
        symbol=re.search(r'&([A-Za-z_][A-Za-z_0-9.]*)',export['expression']).group(1)
        result[symbol]=(count*size,size)
    if role==1:result['lane.kernel.expanded']=(512,4)
    if endpoint:
        result.update({'packets.receive_buffer':(128,4),'packets.transmit_buffer':(128,4)})
    if linear_root or role in (2,3,4,5,6,8,9):result['tx']=(124,4)
    if root and not linear_root or role in (2,3,4,5):result['rows.tx']=(124,4)
    if role in (2,3,4,5):result['retained_chunk']=(256,2)
    if role in (4,8,9):result['receiver.pending_keys']=(16,4)
    if role==6:result.update({'rows.row':(256,2),'frame.buffer':(512,4),'ready_seen':(374,2),'ready_counts':(10,2)})
    if role in (6,7):result['observer.buffer']=(128,4)
    return result
