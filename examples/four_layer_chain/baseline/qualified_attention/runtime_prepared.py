...
class Prepared:
    def __init__(self,root,manifest_pin):
        ...
        require(self.manifest['kind']=='derived_vertical_ROIs_from_accepted_original_words' and
                self.manifest['application']==[29,1160] and self.manifest['all_original_words_reused_checked'],
                'Derived original-word preparation for the actual vertical geometry')
        self.files=self.manifest['files'];require(len(self.files)==55,'46 packed tile groups +3 configuration +6 original parameter/reference files')
        require(self.manifest['packed_matrix_bytes']==751435776,'Complete original Layer3 packed matrices')
    ...
    def transfer(self,item):
        ...
        if 'file_flat_begin' in source:
            start=source['file_flat_begin'];selected=value[start:start+h*w]
        elif 'file_x' in source:
    ...
    def input_row(self,index):
        ...
        require(value.shape==(8,5120) and value.dtype==self.np.dtype('<u2') and index in (0,1),
                'Eight original prompt rows; capture selects rows0/1')
    ...
    def verify_unchanged(self):
        ...
        require(self.verified==self.files.keys() or self.verified==self.files.keys()-{'reference-output.npy'},'Every execution parameter/input consumed; nominal output is audit-only')
