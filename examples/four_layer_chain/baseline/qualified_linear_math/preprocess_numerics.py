...
CHANNELS=384
POLICY={'name':'layer0-preprocess384-expanded-domain-candidate-v1','channels':384,'history_width':4,'epsilon':EPSILON,
 ...
 'domain':'Synthetic original-range extrema: BF16 input abs<=40.75, convweight abs<=1, a/b abs<=25.875, Alog in[-5.5625,-1.0859375], dtbias abs<=0.5; finite normal results or zero; no subnormal/overflow correctness claim',
 'exponential':'custom domain[-48,1] range reduction by splitln2, degree8 FP32 Horner polynomial, exact normal power-of-two scale; gate unchanged',
...
def validate(token,weights,history,observed):
    ...
    conversions['actual_exp_arguments_in_domain']=all(-48<=x<=1 for x in exp_arguments)
