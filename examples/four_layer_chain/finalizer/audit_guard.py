...
import fcntl,hashlib,json,os,resource,shutil,signal,subprocess,sys,time
from finalizer_gate import ROOT,verify,release_gate,atomic_json
    ...
    with (ROOT.parent/'hardware.lock').open('a') as lock:
        ...
        try:
            ...
            with (ROOT/'audit.log').open('x') as log:
                proc=subprocess.Popen([sys.executable,'-B','finalize.py'],cwd=ROOT,env=env,stdout=log,
                ...
                while proc.poll() is None:
                    ...
                    if shutil.disk_usage(ROOT).free<profile['disk_free_reserve_bytes']:raise ValueError('Offline disk reserve')
                    growth=0
