...
from qwen38.resident_sdk_admission import CANDIDATE, PROFILE, STEPS, GENERATED, application_elf_names, compare_compiled_states, validate, stage_limits, THREAD_ENV


ROOT=Path(__file__).resolve().parents[1]
HERE=ROOT/'examples/wp16/resident'
if not HERE.exists():HERE=ROOT
def load(name):
    spec=importlib.util.spec_from_file_location('resident_'+name+'_admission_test',HERE/(name+'.py'))
    module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module);return module


class ResidentSDKAdmissionTests(unittest.TestCase):
    def test_no_compile_path_or_altered_reuse_receipt(self):
        for name in ('entry_sdk','container_entry','compile_sdk'):
            module=load(name)
            with patch.object(sys,'argv',[name+'.py','compile']),self.assertRaises(ValueError):module.main()
        from qwen38 import resident_sdk_admission as admission
        with tempfile.TemporaryDirectory() as tmp:
            work=Path(tmp).resolve()
            (work/'compiled-admission.json').write_text('{}')
            with patch.object(admission,'compiled_state',return_value={}),self.assertRaisesRegex(ValueError,'preserved SDK001'):
                admission.verify_compiled(work)
    ...
    def test_edge_ELFs_stay_distinct_from_exact_application_PE_set(self):
        application = {f'out/bin/out_{rank}_0.elf' for rank in range(8)}
        ...
        for extra in ('out/bin/out_8_0.elf', 'out/bin/unexpected.elf'):
            with self.assertRaises(ValueError):
    ...
    def test_scope_resources_and_admission_are_exact(self):
        ...
        for name, value in [('cpu_affinity', [0, 1]), ('simulation_seconds', 421),
                            ('swap_bytes', 1), ('input_indices', [0, 1]), ('generation_input_contraction', [[1,0,1],[2,1,2],[3,3,2]]), ('pes', 4),
                            ('full_model', True)]:
    ...
    def test_reorder_or_command_change_cannot_bypass_serial_entry(self):
        for change in ('compile', 'argv', 'time'):
            altered = self.admission()
            if change == 'compile':
                altered['steps']['steps'].insert(0,dict(name='compile',seconds=300,argv=['python','compile_sdk.py']))
            elif change == 'argv':
                altered['steps']['steps'][0]['argv'] = ['python', 'driver.py']
            else:
                altered['steps']['steps'][0]['seconds'] = 421
            with self.assertRaises(ValueError):
                validate(altered, 'test-only-not-real')

    def test_distinct_stage_limits_and_changed_profile_refused(self):
        with self.assertRaises(ValueError):stage_limits('compile')
        ...
        for stage in ('simulate',):
            for bad in (False,True):
                with self.subTest(stage=stage,bad=bad),tempfile.TemporaryDirectory() as folder,ExitStack() as stack:
                    ...
                    if bad:actual['memory.max']='1073741824'
    ...
    def test_supervisor_stage_properties_actual_mismatch_and_cleanup(self):
        ...
        for bad_stage in (None,'simulate'):
            with self.subTest(bad_stage=bad_stage),tempfile.TemporaryDirectory() as folder,ExitStack() as stack:
                ...
                def run(command,**kwargs):
                    ...
                    if stage=='simulate':
                        ...
                        (work/'result.json').write_text(json.dumps(dict(status='passed',normal_stop=True)))
                def properties(unit):
                ...
                def read(path,*args,**kwargs):
                    if str(path).startswith('/sys/fs/cgroup/synthetic/'):
                        ...
                        if active['stage']==bad_stage:actual['memory.max']='1073741824'
                ...
                self.assertEqual([name for name,_ in launched],['simulate'])
                for stage,command in launched:
                    self.assertIn('MemoryMax='+stage_limits(stage)['memory.max'],command)
                    self.assertIn('RuntimeMaxSec='+'420',command)
