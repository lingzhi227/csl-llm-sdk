"""Freeze one original-input QK-to-attention composition candidate from qualified CPU fixtures."""
import argparse,ast,hashlib,json,re,shutil,sys
from pathlib import Path
sys.path.insert(0,str(Path(__file__).resolve().parents[1]/'core'))
from qwen38.attention_composed_numerics import POLICY
from qwen38.attention_composed_protocol import LEDGER,validate_ledger
...
def prepare(root,run,image,reference):
    ...
    required={'fixtures.json','oracles.json','numerical-policy.json','trig-qualification.json'}
    ...
    for n in ('pe.csl','layout.csl','driver.py','compile_entry.py'):shutil.copyfile(root/'examples/wp12'/n,run/n)
    shutil.copyfile(root/'csl/kernels/qk_rms256_rope64_bf16.csl',run/'qk_kernel.csl')
    shutil.copyfile(root/'csl/kernels/sincos_bounded_f32.csl',run/'trig_kernel.csl')
    shutil.copyfile(root/'csl/kernels/attention256_bf16.csl',run/'attention_kernel.csl')
    shutil.copyfile(root/'csl/kernels/exp_bounded_f32.csl',run/'exp_kernel.csl')
    for n in ('__init__.py','resources.py','locking.py','elf_inventory.py','rms_numerics.py','gated_numerics.py','interval_numerics.py','attention_numerics.py','qk_rope_numerics.py','qk_rope_protocol.py','rotary_trig.py','attention_composed_numerics.py','attention_composed_protocol.py','attention_protocol.py'):
    ...
    # Cheap delimiter/AST preflight catches generator mistakes before an SDK job.
    for path in run.glob('*.csl'):
        ...
        for char in source:
            if char in '([{':stack.append(char)
                ...
                if not stack or stack.pop()!=pairs[char]:raise ValueError('CSL delimiter mismatch: '+path.name)
        if stack:raise ValueError('Unclosed CSL delimiter: '+path.name)
    for path in run.glob('*.py'):ast.parse(path.read_text())
    (run/'reference-result.json').write_text(json.dumps(result,indent=2)+'\n')
    from qwen38.attention_composed_protocol import budget
    (run/'readback-plan.json').write_text(json.dumps({**budget(),'planning_seconds':195,'hard_seconds':240},indent=2)+'\n')
    ...
    spec={'required_profile':'sdk','planned_write_bytes':134217728,'steps':[
        ...
        {'name':'simulate','seconds':240,'argv':prefix+['driver.py']}]}
    ...
    (run/'source-manifest.json').write_text(json.dumps({'files':files,'scope':'synthetic original Q/K/V/gate through device QK RMS/RoPE and persistent attention'},indent=2)+'\n')
    return run
