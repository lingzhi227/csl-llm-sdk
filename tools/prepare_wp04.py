"""Freeze the audited CPU source-body fixtures; no install, download or execution."""
import argparse
import hashlib
import json
...
SOURCES = {
    ...
    'import_utils.py': 'e147f99cf68352e3260e9780930501c3becc0c40a5ff04150ff641b724161732',
}
...
def prepare(root, run, sources, interpreter):
    ...
    for name in ('__init__.py', 'resources.py', 'locking.py'):
    ...
    shutil.copyfile(root/'examples/wp04/reference_checks.py', run/'reference_checks.py')
    spec = {'planned_write_bytes': 10*1024*1024, 'required_profile': 'cpu-reference',
            'steps': [{'name': 'reference', 'seconds': 60, 'argv': [str(interpreter), 'reference_checks.py']}]}
