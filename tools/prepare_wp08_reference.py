"""Freeze the preprocessing384 CPU source-body fixtures; no install, download or execution."""
import argparse
import hashlib
import json
...
def prepare(root, run, sources, interpreter):
    ...
    for name in ('__init__.py', 'resources.py', 'locking.py', 'rms_numerics.py', 'gated_numerics.py', 'preprocess_numerics.py'):
    ...
    shutil.copyfile(root/'examples/wp08/build_reference.py', run/'build_reference.py')
