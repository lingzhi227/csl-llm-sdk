"""Freeze one standalone gated RMS128 candidate from qualified CPU fixtures."""
import argparse,hashlib,json,shutil,sys
from pathlib import Path
sys.path.insert(0,str(Path(__file__).resolve().parents[1]/'core'))
from qwen38.gated_numerics import POLICY
...
def prepare(root,run,image,reference):
    ...
    if result.get('passed') is not True or len(result.get('checks',[]))!=4:raise ValueError('CPU reference must pass four checks')
    required={'fixtures.json','numerical-policy.json'}
    ...
    for n in ('pe.csl','layout.csl','driver.py','compile_entry.py'):shutil.copyfile(root/'examples/wp07'/n,run/n)
    shutil.copyfile(root/'csl/kernels/gated_rms128_bf16.csl',run/'kernel.csl')
    for n in ('__init__.py','resources.py','locking.py','elf_inventory.py','rms_numerics.py','gated_numerics.py'):
    ...
    (run/'source-manifest.json').write_text(json.dumps({'files':files,'scope':'synthetic standalone BF16 gated RMS128'},indent=2)+'\n')
    return run
