"""Freeze the original QK-to-attention composition original-input CPU source-body fixtures; no install, download or execution."""
import argparse
import hashlib
import json
...
def prepare(root, run, sources, interpreter):
    ...
    for name in ('__init__.py', 'resources.py', 'locking.py', 'rms_numerics.py', 'gated_numerics.py', 'interval_numerics.py', 'attention_numerics.py', 'qk_rope_numerics.py', 'rotary_trig.py', 'attention_composed_numerics.py'):
    ...
    shutil.copyfile(root/'examples/wp12/build_reference.py', run/'build_reference.py')
    spec = {'planned_write_bytes': 32*1024*1024, 'required_profile': 'cpu-reference',
