"""Freeze one QK RMS256/partial RoPE64 candidate from qualified CPU fixtures."""
import argparse,hashlib,json,shutil,sys
from pathlib import Path
sys.path.insert(0,str(Path(__file__).resolve().parents[1]/'core'))
from qwen38.qk_rope_numerics import POLICY
from qwen38.qk_rope_protocol import LEDGER,validate_ledger
...
def prepare(root,run,image,reference):
    ...
    for n in ('pe.csl','layout.csl','driver.py','compile_entry.py'):shutil.copyfile(root/'examples/wp11'/n,run/n)
    shutil.copyfile(root/'csl/kernels/qk_rms256_rope64_bf16.csl',run/'kernel.csl')
    shutil.copyfile(root/'csl/kernels/sincos_bounded_f32.csl',run/'trig_kernel.csl')
    for n in ('__init__.py','resources.py','locking.py','elf_inventory.py','rms_numerics.py','gated_numerics.py','interval_numerics.py','attention_numerics.py','qk_rope_numerics.py','qk_rope_protocol.py','rotary_trig.py'):
    ...
    from qwen38.qk_rope_protocol import budget
    (run/'readback-plan.json').write_text(json.dumps({**budget(),'planning_seconds':130,'hard_seconds':180},indent=2)+'\n')
    ...
    (run/'source-manifest.json').write_text(json.dumps({'files':files,'scope':'synthetic original Q/K RMS256 and bounded device partial RoPE64'},indent=2)+'\n')
    return run
