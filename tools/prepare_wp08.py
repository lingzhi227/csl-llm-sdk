"""Freeze one selected-head preprocessing384 candidate from qualified CPU fixtures."""
import argparse,hashlib,json,shutil,sys
from pathlib import Path
sys.path.insert(0,str(Path(__file__).resolve().parents[1]/'core'))
from qwen38.preprocess_numerics import POLICY
...
def prepare(root,run,image,reference):
    ...
    for n in ('pe.csl','layout.csl','driver.py','compile_entry.py'):shutil.copyfile(root/'examples/wp08'/n,run/n)
    shutil.copyfile(root/'csl/kernels/preprocess384_bf16.csl',run/'kernel.csl')
    shutil.copyfile(root/'csl/kernels/exp_bounded_f32.csl',run/'exp_kernel.csl')
    for n in ('__init__.py','resources.py','locking.py','elf_inventory.py','rms_numerics.py','gated_numerics.py','preprocess_numerics.py','preprocess_protocol.py','resource_ledger.py'):
    ...
    (run/'source-manifest.json').write_text(json.dumps({'files':files,'scope':'synthetic selected-head preprocessing384 and persistent history'},indent=2)+'\n')
    return run
