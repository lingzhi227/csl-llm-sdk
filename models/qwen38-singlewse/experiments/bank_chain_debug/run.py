...
def main():
    ...
    with runtime(args.physical) as (runner,dtype,order):
        ids={n:runner.get_id(n) for n in ['weights','scales','config','input','results','trace','bank_data','debug_state']}
        ...
        for index,tile in enumerate(meta['tiles']):
            x,y=tile['x'],tile['y']
            print(json.dumps({'stage':'weight_upload','tile':index}),flush=True)
        ...
        print(json.dumps({'stage':'start_receivers'}),flush=True)
        runner.launch('start',nonblock=False)
        for call,inputs in enumerate(f['inputs'][:1],1):
            put('input',inputs.copy(),8,4)
            print(json.dumps({'stage':'begin_device_graph'}),flush=True)
            start=time.monotonic();runner.launch('run_test',nonblock=False)
            deadline=time.monotonic()+90;prior=None;progress=[]
            while True:
                state=get('debug_state',8,8,4,kind=np.uint32)
                root_trace=get('trace',6,8,4,kind=np.uint32)
                signature=state.tolist()+root_trace.tolist()
                if signature!=prior:
                    prior=signature;record=dict(stage='device_progress',debug_state=state.tolist(),root_trace=root_trace.tolist(),seconds=time.monotonic()-start)
                    progress.append(record);Path('diagnostic-progress.json').write_text(json.dumps(progress,indent=2)+'\n');print(json.dumps(record),flush=True)
                if root_trace[4]>=call:break
                if time.monotonic()>deadline:raise TimeoutError('Bounded diagnostic graph did not finish')
                time.sleep(1)
            # The root finishes emitting END before every endpoint necessarily
            # observes it; wait for every endpoint without injecting operands.
            while True:
                all_trace=get('trace',6,0,0,9,5,kind=np.uint32).reshape(5,9,6)
                if np.all(all_trace[:,:,4]>=call):break
                if time.monotonic()>deadline:raise TimeoutError('Diagnostic endpoint END receipt did not finish')
                time.sleep(1)
    ...
    Path('result.json').write_text(json.dumps(dict(passed=True,normal_stop=True,physical=args.physical,
        full_model=False,full_matrix=False,scope=meta['scope'],device_controlled=True,retention_pass=True,diagnostic_only=True,full_retention=args.physical,intermediate_never_uploaded=True,bank_capacity=17536,calls=records),indent=2)+'\n')
