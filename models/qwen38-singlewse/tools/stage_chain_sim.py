"""Qualify the staged bank-chain candidate locally while the physical lock is busy."""
...
name='bank-chain-sim-001';dest='/srv/model-storage/qwen38-singlewse/runs/'+name
out=ROOT/'evidence'/name
if a.phase=='stage':
    files={n:(ROOT/'experiments/bank_chain'/n).read_bytes() for n in ['layout.csl','pe.csl','run.py','experiment.json']}
    for n in ['qwen_math.csl','fp8_matrix_block.csl','fp8_codec.csl','fp8_activation.csl']:files[n]=(ROOT/'csl'/n).read_bytes()
    ...
    frozen=json.loads((ROOT/'evidence/bank-chain-hw-001/source-manifest.json').read_text())['files']
    ...
    reader=subprocess.Popen(['sh','/path/to/alcf-session.sh','host','tar -cf - -C /srv/qwen38-singlewse-hardware/bank-chain-hw-001 fixture.npz fixture.json'],stdout=subprocess.PIPE)
    ...
    unit='qwen38-single-chain-'+a.phase+'-001'
