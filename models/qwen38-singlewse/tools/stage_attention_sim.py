"""Stage immutable attention group source and bound every workstation phase."""
...
name='attention-group-sim-'+a.attempt;dest='/srv/model-storage/qwen38-singlewse/runs/'+name
out=ROOT/'evidence'/name
if a.phase=='stage':
    files={n:(ROOT/'experiments/attention_group'/n).read_bytes() for n in ['layout.csl','pe.csl','prepare.py','run.py','experiment.json']}
    for n in ['qwen_math.csl','attention_math.csl','kv_shard64.csl']:files[n]=(ROOT/'csl'/n).read_bytes()
    ...
    if a.phase=='prepare':command=['/usr/bin/python3','-u','prepare.py']
        ...
        command=['/opt/cerebras/sdk/2.10.1/cslc','layout.csl','--arch=wse3','--fabric-dims=12,3','--fabric-offsets=4,1','--memcpy','--channels=1','--max-parallelism=1','-o','out']
    else:command=['/opt/cerebras/sdk/2.10.1/cs_python','run_micro_sim.py']
    unit='qwen38-single-attention-'+a.phase+'-'+a.attempt
