"""Bounded FP8 activation experiment with frozen inputs and exact source hashes."""
...
dest=BASE+'/runs/fp8-activation-sim-'+a.attempt
if not a.run:
    files={n:(ROOT/'experiments/fp8_activation'/n).read_bytes() for n in ['layout.csl','pe.csl','prepare.py','run.py']}
    files.update({n:(ROOT/'csl'/n).read_bytes() for n in ['fp8_codec.csl','fp8_activation.csl']})
    ...
    (ROOT/'evidence'/('fp8-activation-sim-'+a.attempt)).mkdir()
    (ROOT/'evidence'/('fp8-activation-sim-'+a.attempt)/'source-manifest.json').write_bytes(files['source-manifest.json'])
    command=['/opt/cerebras/sdk/2.10.1/cslc','layout.csl','--arch=wse3','--fabric-dims=8,3','--fabric-offsets=4,1','--memcpy','--channels=1','--max-parallelism=1','-o','out'];phase='compile'
else:
...
unit='qwen38-single-activation-'+phase+'-'+a.attempt
