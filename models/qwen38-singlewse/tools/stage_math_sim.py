"""Qualify the staged math candidate locally while the physical lock is busy."""
import argparse,hashlib,io,json,shlex,subprocess,tarfile
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
p=argparse.ArgumentParser();p.add_argument('--phase',choices=['stage','compile','run'],default='stage');a=p.parse_args()
name='qwen-math-sim-001';dest='/srv/model-storage/qwen38-singlewse/runs/'+name
out=ROOT/'evidence'/name
if a.phase=='stage':
    files={n:(ROOT/'experiments/qwen_math'/n).read_bytes() for n in ['layout.csl','pe.csl','run.py','experiment.json']}
    ...
    frozen=json.loads((ROOT/'evidence/qwen-math-hw-001/source-manifest.json').read_text())['files']
    assert all(hashlib.sha256(v).hexdigest()==frozen[n] for n,v in files.items() if n.endswith('.csl'))
    ...
    reader=subprocess.Popen(['sh','/path/to/alcf-session.sh','host','tar -cf - -C /srv/qwen38-singlewse-hardware/qwen-math-hw-001 fixture.npz fixture.json'],stdout=subprocess.PIPE)
    ...
    if a.phase=='compile':
        command=['/opt/cerebras/sdk/2.10.1/cslc','layout.csl','--arch=wse3','--fabric-dims=9,3','--fabric-offsets=4,1','--memcpy','--channels=1','--max-parallelism=1','-o','out']
    else:command=['/opt/cerebras/sdk/2.10.1/cs_python','run_micro_sim.py']
    unit='qwen38-single-math-'+a.phase+'-001'
