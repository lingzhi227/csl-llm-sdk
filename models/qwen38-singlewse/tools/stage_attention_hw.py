"""Freeze the attention candidate and pipe its original fixture between remotes."""
import hashlib,io,json,shlex,subprocess,tarfile
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
name='attention-group-hw-001';dest='/srv/qwen38-singlewse-hardware/'+name
source='/srv/model-storage/qwen38-singlewse/runs/attention-group-sim-001'
frozen=json.loads((ROOT/'evidence/attention-group-sim-001/source-manifest.json').read_text())['files']
files={n:(ROOT/'experiments/attention_group'/n).read_bytes() for n in ['layout.csl','pe.csl','run.py','experiment.json']}
