...
name='bank-chain-debug-sim-001';dest='/srv/model-storage/qwen38-singlewse/runs/'+name
out=ROOT/'evidence'/name
if a.phase=='stage':
    files={n:(ROOT/'experiments/bank_chain_debug'/n).read_bytes() for n in ['layout.csl','pe.csl','run.py','experiment.json']}
    ...
    unit='qwen38-single-chain-debug-'+a.phase+'-001'
    cmd=['systemd-run','--user','--unit='+unit,'--property=WorkingDirectory='+dest,'--property=MemoryMax=2G','--property=MemorySwapMax=0',
        '--property=CPUAffinity=6 7','--property=TasksMax=128','--property=RuntimeMaxSec='+('600' if a.phase=='run' else '300'),'--property=TimeoutStopSec=5','--property=LimitCORE=0',
