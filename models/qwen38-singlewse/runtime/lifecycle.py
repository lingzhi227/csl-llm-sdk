...
def child_limits():
    ...
    full_matrix=config.get('full_matrix',False) and config.get('application_pes')==2600 and config.get('matrix_shape')==[17408,5120]
    full_resident=config.get('full_resident',False)
    if full_resident and (config.get('application_pes')!=870000 or config.get('application')!=[750,1160]):
        raise ValueError('Unexpected full resident resource profile')
    size=(32<<30) if full_resident else (512 if full_matrix else 128)<<20
...
def stage(root, name, script, timeout):
    ...
    try:
        with (root / (name + ".log")).open("x") as log:
            ...
            atomic_json(root / "ACTIVE.json", dict(phase=name, pid=proc.pid, timeout=timeout, source="qwen38-singlewse"))
