"""Bounded physical job lifecycle reused from Qwen HW00; no Qwen execution entry."""
import fcntl
import getpass
import json
import os
...
def child_limits(profile="micro"):
    if profile not in ("micro","full-model"):
        raise ValueError("Unknown bounded execution profile")
    resource.setrlimit(resource.RLIMIT_CORE, (0, 0))
    address_space=(8 if profile=="full-model" else 4)<<30
    file_bytes=(512 if profile=="full-model" else 128)<<20
...
def stage(root, name, script, timeout, profile="micro"):
    if profile not in ("micro","full-model"):
        raise ValueError("Unknown bounded execution profile")
    ...
    try:
        with (root / (name + ".log")).open("x") as log:
            proc = subprocess.Popen([sys.executable, script], cwd=root, env=env, stdout=log,
                                    stderr=subprocess.STDOUT, start_new_session=True, preexec_fn=lambda:child_limits(profile))
            atomic_json(root / "ACTIVE.json", dict(phase=name, pid=proc.pid, timeout=timeout, source="gpt-oss20b"))
            while proc.poll() is None:
                ...
                if status.exists():
                    for line in status.read_text().splitlines():
                        if line.startswith("VmRSS:") and int(line.split()[1]) > (2097152 if profile=="full-model" else 1048576):
                            raise RuntimeError("Host client exceeds declared sampled RSS limit: "+profile)
                time.sleep(1)
        ...
        records = audit["jobs"]
        receipt = dict(stage=name, profile=profile, exit_code=code, error=error, jobs=records,
